...
const bulkSchema = z.object({
  params: pageP,
  body: z.object({ tasks: z.array(taskBody).min(1) }),
});
const updateSchema = z.object({ params: taskP, body: taskBody.partial() });
const reassignSchema = z.object({
  params: taskP,
  body: z.object({ assistant_id: uuid }),
});
...
const groupAllocateSchema = z.object({
  params: z.object({ seriesId: uuid }),
  body: z.object({
    task_ids: z.array(uuid).min(1),
    assistant_ids: z.array(uuid).min(1),
  }),
});
...
router.patch(
  "/:taskId/assign",
  validate(z.object({ params: taskP })),
  ctrl.workflow("assign"),
);
router.patch(
  "/:taskId/start",
  validate(z.object({ params: taskP })),
...
router.patch(
  "/:taskId/submit",
  validate(z.object({ params: taskP })),
  ctrl.workflow("submit"),
);
router.patch(
  "/:taskId/approve",
  validate(z.object({ params: taskP })),
  ctrl.workflow("approve"),
);
router.patch(
  "/:taskId/reject",
  validate(z.object({ params: taskP })),
  ctrl.workflow("reject"),
);
router.patch(
  "/:taskId/cancel",
  validate(z.object({ params: taskP })),
  ctrl.workflow("cancel"),
);
router.patch(
  "/:taskId/reopen",
  validate(z.object({ params: taskP })),
  ctrl.workflow("reopen"),
);
router.patch(
  "/:taskId/complete",
  validate(z.object({ params: taskP })),
...
seriesTaskRouter.post(
  "/group-allocate",
  validate(groupAllocateSchema),
  ctrl.groupAllocate,
);
