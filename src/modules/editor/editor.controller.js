...
const updateSeries = async (req, res, next) => {
  try {
    const data = await seriesRepo.update(req.params.seriesId, { ...req.body, updated_at: new Date().toISOString() });
...
const updateSeriesStatus = async (req, res, next) => {
  try {
    const data = await seriesRepo.update(req.params.seriesId, { status: req.body.status, updated_at: new Date().toISOString() });
...
const seriesWorkflow = (status) => async (req, res, next) => {
  try {
    const data = await seriesRepo.update(req.params.seriesId, { status, updated_at: new Date().toISOString() });
...
const addMember = async (req, res, next) => {
  try {
    ...
    const already = await seriesMembersRepo.findBySeriesAndUser(req.params.seriesId, req.body.user_id);
    if (already) return next(new AppError('User already a member', 409));
    const data = await seriesMembersRepo.create({ series_id: req.params.seriesId, user_id: req.body.user_id, role_in_series: req.body.role_in_series });
...
const updateMember = async (req, res, next) => {
  try {
    const data = await seriesMembersRepo.update(req.params.seriesMemberId, { role_in_series: req.body.role_in_series });
...
const seriesProgress = async (req, res, next) => {
  try { return sendSuccess(res, 200, await dashboardSvc.getSeriesProgress(req.params.seriesId), 'Success'); } catch (e) { next(e); }
};

const chapterProgress = async (req, res, next) => {
  try { return sendSuccess(res, 200, await dashboardSvc.getChapterProgress(req.params.chapterId), 'Success'); } catch (e) { next(e); }
};

const taskSummary = async (req, res, next) => {
  try { return sendSuccess(res, 200, await dashboardSvc.getTaskSummary(), 'Success'); } catch (e) { next(e); }
};

const assistantsWorkload = async (req, res, next) => {
  try { return sendSuccess(res, 200, await dashboardSvc.getAssistantsWorkload(), 'Success'); } catch (e) { next(e); }
};

const assistantPerformance = async (req, res, next) => {
  try { return sendSuccess(res, 200, await dashboardSvc.getAssistantPerformance(req.params.assistantId), 'Success'); } catch (e) { next(e); }
};
...
const listReviewTasks = async (req, res, next) => {
  try {
    ...
    const { data, total } = await taskReviewSvc.listReviewTasks({ status, assistantId: assistant_id, chapterId: chapter_id, seriesId: series_id, offset, limit });
...
const getReviewTaskById = async (req, res, next) => {
  try { return sendSuccess(res, 200, await taskReviewSvc.getTaskById(req.params.taskId), 'Success'); } catch (e) { next(e); }
};

const getReviewTaskDetail = async (req, res, next) => {
  try { return sendSuccess(res, 200, await taskReviewSvc.getTaskDetail(req.params.taskId), 'Success'); } catch (e) { next(e); }
};

const taskWorkflow = (action) => async (req, res, next) => {
  try {
    const data = await taskReviewSvc.performWorkflow(req.params.taskId, req.user.user_id, action, req.body);
...
const bulkApproveTasks = async (req, res, next) => {
  try {
    await taskReviewSvc.bulkUpdateTasks(req.body.task_ids, 'approved', req.user.user_id, req.body.note);
...
const bulkRejectTasks = async (req, res, next) => {
  try {
    await taskReviewSvc.bulkUpdateTasks(req.body.task_ids, 'rejected', req.user.user_id, req.body.content);
...
const bulkRequestRevision = async (req, res, next) => {
  try {
    await taskReviewSvc.bulkUpdateTasks(req.body.task_ids, 'needs_revision', req.user.user_id, req.body.content);
...
const bulkUpdateTaskStatus = async (req, res, next) => {
  try {
    await taskReviewSvc.bulkUpdateTasks(req.body.task_ids, req.body.status, req.user.user_id, req.body.note);
...
const listReviewManuscripts = async (req, res, next) => {
  try {
    ...
    const { data, total } = await manuscriptReviewSvc.listManuscripts({ status, seriesId: series_id, chapterId: chapter_id, offset, limit });
    return res.status(200).json({ success: true, message: 'Success', data, pagination: buildPaginationMeta(page, limit, total) });
  } catch (e) { next(e); }
};

const getReviewManuscriptById = async (req, res, next) => {
  try { return sendSuccess(res, 200, await manuscriptReviewSvc.getManuscriptById(req.params.manuscriptId), 'Success'); } catch (e) { next(e); }
};

const getReviewManuscriptDetail = async (req, res, next) => {
  try { return sendSuccess(res, 200, await manuscriptReviewSvc.getManuscriptDetail(req.params.manuscriptId), 'Success'); } catch (e) { next(e); }
};

const manuscriptWorkflow = (action) => async (req, res, next) => {
  try {
    const data = await manuscriptReviewSvc.performWorkflow(req.params.manuscriptId, req.user.user_id, action, req.body);
    return sendSuccess(res, 200, data || null, `Manuscript ${action} successful`);
  } catch (e) { next(e); }
};

const bulkApproveManuscripts = async (req, res, next) => {
  try {
    await manuscriptReviewSvc.bulkUpdateManuscripts(req.body.manuscript_ids, 'approved');
...
const bulkRejectManuscripts = async (req, res, next) => {
  try {
    await manuscriptReviewSvc.bulkUpdateManuscripts(req.body.manuscript_ids, 'rejected');
...
const bulkUpdateManuscriptStatus = async (req, res, next) => {
  try {
    await manuscriptReviewSvc.bulkUpdateManuscripts(req.body.manuscript_ids, req.body.status);
...
const updateAnnotationStatus = async (req, res, next) => {
  try {
    const data = await annotationsRepo.update(req.params.annotationId, { status: req.body.status, updated_at: new Date().toISOString() });
    return sendSuccess(res, 200, data, 'Status updated');
  } catch (e) { next(e); }
};
...
const createFeedback = async (req, res, next) => {
  try {
    const data = await pageTaskFeedbacksRepo.create({ ...req.body, task_id: req.params.taskId, mangaka_id: req.user.user_id });
...
const updateFeedback = async (req, res, next) => {
  try {
    const data = await pageTaskFeedbacksRepo.update(req.params.feedbackId, { ...req.body, updated_at: new Date().toISOString() });
...
const updateFeedbackStatus = async (req, res, next) => {
  try {
    const data = await pageTaskFeedbacksRepo.update(req.params.feedbackId, { status: req.body.status, updated_at: new Date().toISOString() });
...
const createReviewSession = async (req, res, next) => {
  try {
    const data = await reviewSessionsRepo.create({ ...req.body, created_by_user_id: req.user.user_id, status: 'pending' });
...
const reviewSessionWorkflow = (action) => async (req, res, next) => {
  try {
    const WORKFLOW = { start: 'in_progress', pause: 'paused', complete: 'completed', finish: 'finished', cancel: 'cancelled' };
    const updates = { status: WORKFLOW[action], updated_at: new Date().toISOString() };
    if (action === 'start') updates.started_at = new Date().toISOString();
    if (['complete', 'finish'].includes(action)) updates.ended_at = new Date().toISOString();
...
const createProposal = (proposalType) => async (req, res, next) => {
  try {
    const data = await reviewSessionsRepo.create({ ...req.body, created_by_user_id: req.user.user_id, status: 'pending', name: req.body.name || `${proposalType} Proposal` });
...
module.exports = {
  listSeries, getSeriesById, getSeriesDetail, updateSeries, updateSeriesStatus, seriesWorkflow,
  listMembers, addMember, updateMember, removeMember,
  dashboardOverview, seriesProgress, chapterProgress, taskSummary, assistantsWorkload, assistantPerformance,
  listReviewTasks, getReviewTaskById, getReviewTaskDetail, taskWorkflow,
  bulkApproveTasks, bulkRejectTasks, bulkRequestRevision, bulkUpdateTaskStatus,
  listReviewManuscripts, getReviewManuscriptById, getReviewManuscriptDetail, manuscriptWorkflow,
  bulkApproveManuscripts, bulkRejectManuscripts, bulkUpdateManuscriptStatus,
  listAnnotationsByPage, listAnnotationsByRegion, listAnnotationsByTask, getAnnotationById, createAnnotation, updateAnnotation, updateAnnotationStatus, deleteAnnotation,
  listTaskFeedbacks, createFeedback, updateFeedback, updateFeedbackStatus, deleteFeedback,
  listReviewSessions, getReviewSessionById, getReviewSessionDetail, createReviewSession, updateReviewSession, reviewSessionWorkflow,
  createProposal,
