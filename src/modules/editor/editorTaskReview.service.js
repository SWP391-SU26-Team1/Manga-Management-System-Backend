...
const getTaskDetail = async (taskId) => {
  ...
  const [{ data: annotations }, { data: feedbacks }] = await Promise.all([
    ...
    supabase.from('page_task_feedback').select('*').eq('task_id', taskId).order('created_at'),
  ]);
...
const performWorkflow = async (taskId, editorId, action, payload) => {
  ...
  if (action === 'override-status') {
    if (!payload.status) throw new AppError('status required for override', 400);
    return pageTasksRepo.update(taskId, { status: payload.status, updated_at: new Date().toISOString() });
  }
  ...
  if (action === 'request-revision') {
    ...
    if (payload.content) {
      await pageTaskFeedbacksRepo.create({ task_id: taskId, mangaka_id: editorId, assistant_id: task.assistant_id, feedback_content: payload.content, feedback_type: 'revision_request', status: 'pending' });
    }
  ...
  if (action === 'approve' || action === 'reject') {
    if (task.assistant_id) {
      await createNotification(task.assistant_id, `Task ${action}d`, `Your task has been ${action}d.`, `task_${action}d`);
    }
  }
  return updated;
};
...
const bulkUpdateTasks = async (taskIds, status, editorId, content) => {
  const updates = taskIds.map((id) => supabase.from('page_task').update({ status, updated_at: new Date().toISOString() }).eq('task_id', id));
  ...
  if (['rejected', 'needs_revision'].includes(status) && content) {
    ...
    const feedbacks = (tasks || []).map((t) => ({ task_id: t.task_id, mangaka_id: editorId, assistant_id: t.assistant_id, feedback_content: content, feedback_type: 'bulk_review', status: 'pending' }));
...
module.exports = { listReviewTasks, getTaskById, getTaskDetail, performWorkflow, bulkUpdateTasks };
